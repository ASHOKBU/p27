...
class Topic(models.Model):
    topic_name= models.CharField(max_length=200, unique=True,blank=False,validators=[validators.MaxLengthValidator(10)])
    def __str__(self):
        return self.topic_name
def validate_name(name):
    if not name.isalpha():
        raise ValidationError('{} is having characters are other than alphabets'.format(name))
